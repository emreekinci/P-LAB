# 01.04.2020
# N tane sayı ile liste oluşturma
# 2 farklı method ile histogram
# Bir listedeki mod'u hash mapping veya liste üzerinde aramayla bulma
# Lineer arama
# Listeyi baştan sonra sıralama
# Sıralanmış bir listede Binary arama
# Listedeki elemanların ortalaması(aritmetik geometrik), Mod'u, Medyan'ı(Liste üzerinde istatistiklerin bulunması.
...
def get_n_random_numbers(n=10,min_=-5,max_=5): #random modülü ile fonksiyon
    numbers=[]
    for i in range(n):
        numbers.append(random.randint(min_,max_))#döngüde random fonksiyonu ile her seferinde dizimize ekleme yaptık..
    return numbers

my_list = get_n_random_numbers(12,-100,50)       #ilk değer 12 sayı,ikinci ve üçüncü değerler ise aralığımızı veriyor.

sorted(my_list)#listemizi sıraladık..

def my_frequency_with_dict(my_list):             # Hash Mapping Yöntemi ile işlem yapma.
    frequency_dict={}
    for item in my_list:
        if (item in frequency_dict): #item frekans tablosunda varsa 1 arttır.
            frequency_dict[item]=frequency_dict[item]+1
        else:                        #yoksa demekki ilk defa karşılaşıyoruz +1.
